...
async function setData(channelName, context, params) {
  ...
  await updateChannelProperty(
    channelName,
    params[0],
    eval(params.splice(1).join(" "))
  );
}
...
const pajbotLinkRegex = new RegExp(
  "\\(?(?:(http|https):\\/\\/)?(?:((?:[^\\W\\s]|\\.|-|[:]{1})+)@{1})?" +
...
async function censorBanphrases(channelName, message) {
  ...
  if (
    channelProtectionData == null ||
    channelProtectionData["endpoint"] == null
  ) {
  ...
  do {
    ...
    try {
      ...
      if (banned) {
        ...
        console.log(`Built regex: >${regex}< flags >${flags}<`);
    ...
    lock.enter(token => {
      ...
      (async () => {
        ...
        if (config.modChannels.includes(channelName)) {
          // don't sleep!
          console.log(`immediately unlocking ${channelName} again`);
          token.leave();
          resolve();
          return;
        }
...
async function onMessageHandler(target, context, msg, self) {
  ...
  let disabledCommands =
    (channelConfig.protection || {}).disabledCommands || [];
